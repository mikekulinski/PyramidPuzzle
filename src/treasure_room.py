...
class TreasureRoom(Puzzle):
    ...
    def create_objects(self):
        ...
        icons = ["./data/guitar.png", "./data/bass.png", "./data/drums.png", "./data/piano.png"]
        init_positions = [(1, 1), (1, 7), (7, 1), (7, 7)]
        final_positions = [(2, 4), (3, 4), (5, 4), (6, 4)]
        colors = [
            Color(hsv=(0, .5, 1)),  # Green
            Color(hsv=(.15, .5, 1)),  # Red
            Color(hsv=(.25, .5, 1)),  # Yellow
            Color(hsv=(.5, .5, 1)),  # Blue
        ]
    ...
    def move_block(self,new_location,x,y):
        ...
        if self.is_valid_pos(obj_loc) and self.valid_block_move(obj_loc, self.objects[new_location].move_range):
            ...
            obj = MovingBlock(self.objects[new_location].size, self.grid.grid_to_pixel(obj_loc),self.objects[new_location].move_range, self.objects[new_location].color, self.objects[new_location].icon_source, self.objects[new_location].final_position)
    ...
    def on_player_input(self, button):
        if button in [Button.UP, Button.DOWN, Button.LEFT, Button.RIGHT]:
            ...
            if self.blocks_placed == 4:
                self.get_tile((4,4)).set_color(color=Tile.base_color, source="./data/treasure.png")
