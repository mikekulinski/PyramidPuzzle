...
class BassPuzzle(Puzzle):
	def __init__(self, center_room):
		...
		self.string_pitches = [[48,49,50,51,52,53,54], [52,53,54,55,56,57,58], [56,57,58,59,60,61,62], [60,61,62,63,64,65,66]]
		self.place_objects()
	...
	def place_objects(self):
		...
		for stringp in range(len(self.string_position_start)):
			...
			self.objects[self.string_position_start[stringp]] = BassString((pixel_pos_start, pixel_pos_end),5,(self.string_position_start[stringp], self.string_position_end[stringp]), self.string_pitches[stringp], stringp)
		...
		for f in range(len(self.fret_positions)):
			...
			self.objects[fret] = FretBlock(self.fsize,fret, pos, self.block_interact,move_range, self.correct_blocks[f], "./data/boulder.jpg")
	...
	def move_block(self,new_location,x,y):
		...
		if Puzzle.is_valid_pos(self, obj_loc) and self.valid_block_move(obj_loc, self.objects[new_location].move_range):
			...
			obj = FretBlock(self.fsize, obj_loc, self.grid.grid_to_pixel(obj_loc), self.block_interact, self.objects[new_location].move_range,self.objects[new_location].correct_fret,"./data/boulder.jpg")
...
#	def check_correct_block(self):
	...
	def on_player_input(self, button):
		if button in [Button.UP, Button.DOWN, Button.LEFT, Button.RIGHT]:
			...
			if self.character.grid_pos in self.objects:
				obj = self.objects[(self.character.grid_pos[0], self.character.grid_pos[1])]
	...
	def play_string(self, string):
		...
		if stringidx == 0:
			self.winning[stringidx] = [self.winning[stringidx][0], self.winning[stringidx][0]]
		else:
			if self.winning[stringidx-1][1]:
				self.winning[stringidx] = [self.winning[stringidx][0], self.winning[stringidx-1][1]]
			else:
...
class FretBlock(Tile):
	def __init__(self, size,grid_pos, pos, on_interact, move_range, correct_fret, icon_source):
