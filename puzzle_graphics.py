...
notes = ((.25, 60), (.25, 72), (.25, 71), (.25, 67), (.25, 69), (.25, 71), (.25, 0), (.25, 72), )
...
class MusicBar(InstructionGroup):
    def __init__(self, notes_list):
        ...
        for line in self.staff_lines:
            self.add(line)
        self.clef = Rectangle(source='treble_clef_white.png', pos=(30, self.height), size=(70, self.height / 3.75))
        self.add(self.clef)
        self.time = 0
    ...
    def place_notes(self):
        ...
        for i in range(num_measures):
            ...
            while measure_beats < 1:
                ...
                self.add(Ellipse(size = (50,50), pos = (x_start + (measure_beats) * (x_end - x_start), self.height)))
